import{a as e,c as t,d as n,i as r,l as i,n as a,o,r as s,s as c}from"./supabase.BampuV_k.js";var l=n(r,s,{auth:{persistSession:!1,autoRefreshToken:!1}}),u=document.getElementById(`login-gate`),d=document.getElementById(`dashboard`),f=[],p=[],m=[],h={},g=[],_=[],v=[];async function y(){let[e,n,r,a,s,l]=await Promise.all([i.from(`guides`).select(`*`).order(`created_at`),i.from(`bookings`).select(`*`).order(`date`),i.from(`reviews`).select(`*`).order(`created_at`,{ascending:!1}),i.from(`availability`).select(`guide_id, date`),i.from(`call_slots`).select(`*`).order(`date`),i.from(`call_bookings`).select(`*`).order(`slot_date`)]);f=(e.data||[]).map(c),p=(n.data||[]).map(o),v=n.data||[],m=(r.data||[]).map(t),h={},(a.data||[]).forEach(e=>{(h[e.guide_id]=h[e.guide_id]||[]).push(e.date)}),g=s.data||[],_=l.data||[]}async function b(){await y(),u.classList.add(`is-hidden`),d.classList.remove(`is-hidden`),ae(),Q(),Se(),ye()}(async()=>{await e()&&await b()})(),document.getElementById(`login-form`).addEventListener(`submit`,async t=>{t.preventDefault();let n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pass`).value,a=document.getElementById(`login-error`);a.classList.add(`is-hidden`);let{error:o}=await i.auth.signInWithPassword({email:n,password:r});if(o||!await e()){await i.auth.signOut(),a.classList.remove(`is-hidden`);return}await b()}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await i.auth.signOut(),u.classList.remove(`is-hidden`),d.classList.add(`is-hidden`)});var x=document.getElementById(`modal-overlay`),S=document.getElementById(`photo-input`),C=document.getElementById(`photo-empty`),ee=document.getElementById(`photo-editor`),te=document.getElementById(`photo-result`),w=document.getElementById(`photo-crop-img`),T=document.getElementById(`photo-crop`),E=document.getElementById(`photo-zoom`),D=document.getElementById(`photo-result-circle`),O={x:0,y:0,scale:1,dragging:!1,startX:0,startY:0,origX:0,origY:0,imgSrc:``};function k(e){C.classList.toggle(`is-hidden`,e!==`empty`),ee.classList.toggle(`is-hidden`,e!==`editor`),te.classList.toggle(`is-hidden`,e!==`result`)}function A(){w.style.transform=`translate(${O.x}px, ${O.y}px) scale(${O.scale})`}function j(){let e=w.naturalWidth*O.scale,t=w.naturalHeight*O.scale,n=160-e,r=160-t;O.x=Math.min(0,Math.max(n,O.x)),O.y=Math.min(0,Math.max(r,O.y))}document.getElementById(`photo-btn`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-placeholder`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-change-btn`).addEventListener(`click`,()=>S.click()),S.addEventListener(`change`,()=>{let e=S.files?.[0];if(!e)return;let t=new FileReader;t.onload=e=>{O.imgSrc=e.target?.result,w.src=O.imgSrc,w.onload=()=>{O.scale=160/Math.min(w.naturalWidth,w.naturalHeight),O.x=(160-w.naturalWidth*O.scale)/2,O.y=(160-w.naturalHeight*O.scale)/2,E.min=String(Math.round(O.scale*100)),E.max=String(Math.round(O.scale*100*3)),E.value=String(Math.round(O.scale*100)),A(),k(`editor`)}},t.readAsDataURL(e)}),T.addEventListener(`mousedown`,e=>{e.preventDefault(),O.dragging=!0,O.startX=e.clientX,O.startY=e.clientY,O.origX=O.x,O.origY=O.y,T.style.cursor=`grabbing`}),window.addEventListener(`mousemove`,e=>{O.dragging&&(O.x=O.origX+(e.clientX-O.startX),O.y=O.origY+(e.clientY-O.startY),j(),A())}),window.addEventListener(`mouseup`,()=>{O.dragging=!1,T.style.cursor=`grab`}),T.addEventListener(`touchstart`,e=>{e.touches.length===1&&(O.dragging=!0,O.startX=e.touches[0].clientX,O.startY=e.touches[0].clientY,O.origX=O.x,O.origY=O.y)},{passive:!0}),window.addEventListener(`touchmove`,e=>{!O.dragging||e.touches.length!==1||(O.x=O.origX+(e.touches[0].clientX-O.startX),O.y=O.origY+(e.touches[0].clientY-O.startY),j(),A())},{passive:!0}),window.addEventListener(`touchend`,()=>{O.dragging=!1}),E.addEventListener(`input`,()=>{let e=O.scale;O.scale=parseInt(E.value)/100;let t=O.scale/e;O.x=80-(80-O.x)*t,O.y=80-(80-O.y)*t,j(),A()}),T.addEventListener(`wheel`,e=>{e.preventDefault();let t=parseInt(E.min)/100,n=parseInt(E.max)/100,r=e.deltaY>0?-.05:.05,i=O.scale;O.scale=Math.min(n,Math.max(t,O.scale+r));let a=O.scale/i;O.x=80-(80-O.x)*a,O.y=80-(80-O.y)*a,j(),E.value=String(Math.round(O.scale*100)),A()},{passive:!1}),document.getElementById(`photo-crop-cancel`).addEventListener(`click`,()=>{k(`empty`),S.value=``}),document.getElementById(`photo-crop-confirm`).addEventListener(`click`,()=>{let e=document.createElement(`canvas`);e.width=320,e.height=320,e.getContext(`2d`).drawImage(w,O.x*2,O.y*2,w.naturalWidth*O.scale*2,w.naturalHeight*O.scale*2);let t=e.toDataURL(`image/jpeg`,.9);D.style.backgroundImage=`url(${t})`,k(`result`)});function M(){return f}function N(){return p}function ne(e){return h[e]||[]}function P(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var re=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`],ie=[`Lu`,`Ma`,`Me`,`Je`,`Ve`,`Sa`,`Di`];function F(e,t,n,r){let i=new Set(ne(t)),a=new Set(N().filter(e=>e.guideId===t&&e.status!==`reassigned`).map(e=>e.date)),o=new Date(r,n,1).getDay(),s=o===0?6:o-1,c=new Date(r,n+1,0).getDate(),l=new Date,u=ie.map(e=>`<span class="adm-cal__day-label">${e}</span>`).join(``);for(let e=0;e<s;e++)u+=`<span class="adm-cal__empty"></span>`;for(let e=1;e<=c;e++){let t=`${r}-${String(n+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,o=a.has(t),s=i.has(t),c=new Date(r,n,e)<new Date(l.getFullYear(),l.getMonth(),l.getDate()),d=`adm-cal__day`;c?d+=` adm-cal__day--past`:o?d+=` adm-cal__day--booked`:s?d+=` adm-cal__day--avail`:d+=` adm-cal__day--busy`,u+=`<span class="${d}">${e}</span>`}let d=e.querySelector(`.adm-cal__grid`),f=e.querySelector(`.adm-cal__title`);d&&(d.innerHTML=u),f&&(f.textContent=`${re[n]} ${r}`);let p=e.querySelector(`[data-cal-nav]`);p&&(p.dataset.month=n.toString(),p.dataset.year=r.toString())}function ae(){let e=document.getElementById(`panel-guides`);e.innerHTML=``,M().forEach(t=>e.appendChild(I(t))),L(),Se()}function I(e){let t=e.city===`medina`?`Medine`:`La Mecque`,n=(e.firstName?.[0]||``)+(e.lastName?.[0]||``),r=e.photo?`<div class="adm-g__avatar adm-g__avatar--photo" style="background-image:url(${e.photo})"></div>`:`<div class="adm-g__avatar adm-g__avatar--${e.city}">${P(n)}</div>`,a=N().filter(t=>t.guideId===e.id&&t.status!==`reassigned`),o=a.filter(e=>e.status===`pending-refusal`),s=o.length>0,c=a.length>0,l=document.createElement(`div`);l.className=`adm-g`,l.dataset.guideId=e.id;let u=``;u=s?`<div class="adm-g__notif">
    ...
    `,l.querySelector(`[data-open-creds="${e.id}"]`).addEventListener(`click`,()=>{document.getElementById(`creds-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelector(`[data-delete-guide="${e.id}"]`).addEventListener(`click`,async()=>{if(!confirm(`Supprimer ${e.firstName} ${e.lastName} ?`))return;let{error:t}=await i.from(`guides`).delete().eq(`id`,e.id);if(t){alert(`Erreur lors de la suppression. Réessayez.`);return}f=f.filter(t=>t.id!==e.id),l.remove(),L()}),l.querySelector(`[data-edit-guide="${e.id}"]`).addEventListener(`click`,()=>{se(e.id)});let h=l.querySelector(`[data-toggle-bookings="${e.id}"]`);h&&h.addEventListener(`click`,()=>{document.getElementById(`bookings-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-bk-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-bk-row]`);if(!t)return;let n=t.querySelector(`.adm-g__bk-detail`),r=e.querySelector(`.adm-g__bk-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})});let g=l.querySelector(`[data-toggle-refusal="${e.id}"]`);return g&&g.addEventListener(`click`,()=>{document.getElementById(`refusal-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-rcard]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-rcard]`);if(!t)return;let n=t.querySelector(`.adm-g__rcard-body`),r=e.querySelector(`.adm-g__rcard-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-cal`).forEach(e=>{let t=e.querySelector(`[data-cal-nav]`);if(!t)return;let n=t.dataset.guideId;F(e,n,parseInt(t.dataset.month),parseInt(t.dataset.year));let r=e.querySelector(`[data-cal-prev]`),i=e.querySelector(`[data-cal-next]`);r&&r.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r--,r<0&&(r=11,i--),F(e,n,r,i)}),i&&i.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r++,r>11&&(r=0,i++),F(e,n,r,i)})}),l.querySelectorAll(`[data-toggle-ref-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-ref-row]`);if(!t)return;let n=t.querySelector(`.adm-g__ref-detail`),r=e.querySelector(`.adm-g__ref-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-g__rcard-radio`).forEach(e=>{e.addEventListener(`click`,e=>{e.stopPropagation()})}),l.querySelectorAll(`[data-confirm-refusal]`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t,r=n.dataset.bookingId,a=n.dataset.radioName,o=l.querySelector(`input[name="${a}"]:checked`),s=o?o.value:null,c=N().find(e=>e.id===r);if(!c)return;n.disabled=!0;let{error:u}=await i.from(`bookings`).update({status:`reassigned`}).eq(`id`,r);if(u){n.disabled=!1,alert(`Une erreur est survenue. Réessayez.`);return}if(s){let t=M().find(t=>t.id===e.id),{error:n}=await i.from(`bookings`).insert({guide_id:s,date:c.date,slot:c.slot||null,activity_slug:c.activitySlug||null,activity_name:c.activityName||null,client_name:c.clientName||null,client_email:c.clientEmail||null,client_phone:c.clientPhone||null,status:`confirmed`,replacement_of:t?`${t.firstName} ${t.lastName}`:null});n&&alert(`Le refus a été confirmé, mais la réassignation a échoué (date déjà prise ?).`)}await y(),ae()})}),l}function L(){let e=document.querySelector(`.adm-top__sub`);if(e){let t=M().length;e.textContent=`${t} guide${t===1?``:`s`} enregistre${t===1?``:`s`}`}}var R=null,oe=document.getElementById(`modal-title`),z=document.getElementById(`add-guide-form`);function se(e){let t=M().find(t=>t.id===e);if(!t)return;R=e,oe.textContent=`Modifier le guide`,z.querySelector(`[name="prenom"]`).value=t.firstName||``,z.querySelector(`[name="nom"]`).value=t.lastName||``,z.querySelector(`[name="ville"]`).value=t.city||``,z.querySelector(`[name="gender"]`).value=t.gender||`homme`,z.querySelector(`[name="prix"]`).value=t.pricePerDay?String(t.pricePerDay):``,z.querySelector(`[name="service_prix"]`).value=t.servicePrice?String(t.servicePrice):``,z.querySelector(`[name="supplement_prix"]`).value=t.supplement710?String(t.supplement710):``,z.querySelector(`[name="diplome"]`).value=t.diploma||``,z.querySelector(`[name="langues"]`).value=(t.languages||[]).join(`, `),z.querySelector(`[name="description"]`).value=t.description||``,z.querySelectorAll(`[name="services"]`).forEach(e=>{e.checked=(t.services||[]).includes(e.value)});let n=z.querySelector(`[name="identifiant"]`),r=z.querySelector(`[name="mdp"]`);n.value=t.login||``,n.disabled=!0,r.value=``,r.placeholder=`Non modifiable ici`,r.disabled=!0,t.photo?(D.style.backgroundImage=`url(${t.photo})`,k(`result`)):k(`empty`),x.classList.remove(`is-hidden`)}function B(){R=null,oe.textContent=`Ajouter un guide`,z.reset();let e=z.querySelector(`[name="identifiant"]`),t=z.querySelector(`[name="mdp"]`);e.disabled=!1,t.disabled=!1,t.placeholder=`mot de passe (6 caractères min.)`,k(`empty`),D.style.backgroundImage=``}document.getElementById(`open-add-guide`).addEventListener(`click`,()=>{B(),x.classList.remove(`is-hidden`)}),document.getElementById(`close-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),B()}),document.getElementById(`cancel-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),B()}),x.addEventListener(`click`,e=>{e.target===x&&(x.classList.add(`is-hidden`),B())}),z.addEventListener(`submit`,async e=>{e.preventDefault();let t=z,n=t.querySelector(`[name="prenom"]`).value.trim(),r=t.querySelector(`[name="nom"]`).value.trim(),o=t.querySelector(`[name="ville"]`).value,s=t.querySelector(`[name="gender"]`).value,u=t.querySelector(`[name="prix"]`).value,d=t.querySelector(`[name="service_prix"]`).value,p=t.querySelector(`[name="supplement_prix"]`).value,m=t.querySelector(`[name="diplome"]`).value.trim(),h=t.querySelector(`[name="langues"]`).value.trim(),g=t.querySelector(`[name="description"]`).value.trim(),_=t.querySelector(`[name="identifiant"]`).value.trim().toLowerCase(),v=t.querySelector(`[name="mdp"]`).value;if(!n||!r)return;let y=t.querySelector(`.adm-modal__submit`);y.disabled=!0,y.textContent=`Enregistrement...`;let b=()=>{y.disabled=!1,y.textContent=`Enregistrer`},S=D.style.backgroundImage.match(/url\("?([^"]*)"?\)/),C=S?S[1]:``,ee=Array.from(t.querySelectorAll(`[name="services"]:checked`)).map(e=>e.value),te={first_name:n,last_name:r,city:o||`medina`,gender:s||`homme`,diploma:m||null,description:g||null,languages:h?h.split(`,`).map(e=>e.trim()).filter(Boolean):[],price_per_day:u?parseInt(u):0,service_price:d?parseInt(d):0,supplement_7_10:p?parseInt(p):0,services:ee};if(R){let e=M().find(e=>e.id===R);if(!e){b();return}!C&&e.photo&&(C=e.photo);let{data:t,error:n}=await i.from(`guides`).update({...te,photo:C||null}).eq(`id`,R).select().single();if(b(),n||!t){alert(`Erreur lors de la mise à jour. Réessayez.`);return}let r=c(t),a=f.findIndex(e=>e.id===R);a!==-1&&(f[a]=r);let o=document.querySelector(`[data-guide-id="${R}"]`);if(o){let e=I(r);o.replaceWith(e),e.scrollIntoView({behavior:`smooth`,block:`center`})}}else{if(!_||!/^[a-z0-9._-]+$/.test(_)){b(),alert(`Identifiant invalide : lettres minuscules, chiffres, points, tirets uniquement (sans espaces ni accents).`);return}if(!v||v.length<6){b(),alert(`Le mot de passe doit contenir au moins 6 caractères.`);return}let{data:e,error:t}=await l.auth.signUp({email:_+a,password:v});if(t||!e.user){b(),alert(`Impossible de créer le compte du guide : ${t?.message||`erreur inconnue`}.\n(Identifiant déjà utilisé ?)`);return}let{data:n,error:r}=await i.from(`guides`).insert({...te,photo:C||null,login:_,auth_user_id:e.user.id}).select().single();if(b(),r||!n){alert(`Le compte a été créé mais l'enregistrement du guide a échoué. Réessayez.`);return}let o=c(n);f.push(o);let s=document.getElementById(`panel-guides`),u=I(o);s.appendChild(u),u.scrollIntoView({behavior:`smooth`,block:`center`})}L(),x.classList.add(`is-hidden`),B()});var ce=document.querySelectorAll(`[data-nav-tab]`),le=document.getElementById(`panel-home`),ue=document.getElementById(`panel-guides-header`),de=document.getElementById(`panel-guides`),fe=document.getElementById(`panel-reviews`);ce.forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navTab;ce.forEach(e=>e.classList.remove(`adm-side__link--active`)),e.classList.add(`adm-side__link--active`),le.classList.add(`is-hidden`),ue.classList.add(`is-hidden`),de.classList.add(`is-hidden`),fe.classList.add(`is-hidden`),n===`home`?(le.classList.remove(`is-hidden`),ye()):n===`reviews`?(fe.classList.remove(`is-hidden`),$()):(ue.classList.remove(`is-hidden`),de.classList.remove(`is-hidden`))})});var V=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`];function H(e){return(Math.round(e)/100).toLocaleString(`fr-FR`,{minimumFractionDigits:0,maximumFractionDigits:2})+` €`}function U(e){return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`}function W(e){let[t,n,r]=e.split(`-`).map(Number);return`${r} ${V[n-1].toLowerCase()} ${t}`}function pe(){let e=[];return v.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),_.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),e}function me(){let e=pe(),t=new Date,n=U(t),r=new Date(t),i=(t.getDay()+6)%7;r.setDate(t.getDate()-i);let a=U(r),o=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-01`,s=0,c=0,l=0,u=0;e.forEach(e=>{u+=e.amount,e.date===n&&(s+=e.amount),e.date>=a&&(c+=e.amount),e.date>=o&&(l+=e.amount)});let d=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=H(t))};d(`stat-today`,s),d(`stat-week`,c),d(`stat-month`,l),d(`stat-total`,u),he()}var G=`day`;function he(){let e=document.getElementById(`stat-breakdown`);if(!e)return;let t=pe(),n={};t.forEach(e=>{if(!e.date)return;let[t,r,i]=e.date.split(`-`).map(Number),a=``,o=``,s=``;if(G===`day`)a=e.date,o=W(e.date),s=e.date;else if(G===`month`)a=`${t}-${String(r).padStart(2,`0`)}`,o=`${V[r-1]} ${t}`,s=a;else{let e=new Date(t,r-1,i),n=(e.getDay()+6)%7;e.setDate(e.getDate()-n);let c=U(e);a=c,o=`Semaine du ${W(c)}`,s=c}n[a]||(n[a]={label:o,sort:s,amount:0}),n[a].amount+=e.amount});let r=Object.values(n).sort((e,t)=>t.sort.localeCompare(e.sort));if(r.length===0){e.innerHTML=`<p class="adm-breakdown__empty">Aucun encaissement pour le moment.</p>`;return}e.innerHTML=r.map(e=>`
      <div class="adm-breakdown__row">
        <span class="adm-breakdown__label">${Z(e.label)}</span>
        <span class="adm-breakdown__amount">${H(e.amount)}</span>
      </div>`).join(``)}document.querySelectorAll(`[data-stat-period]`).forEach(e=>{e.addEventListener(`click`,()=>{G=e.dataset.statPeriod,document.querySelectorAll(`[data-stat-period]`).forEach(e=>e.classList.remove(`adm-seg__btn--active`)),e.classList.add(`adm-seg__btn--active`),he()})});var K={year:0,month:0,editing:!1,selectedDay:null};function q(){let e={};return g.forEach(t=>{(e[t.date]=e[t.date]||[]).push({id:t.id,time:t.time})}),Object.values(e).forEach(e=>e.sort((e,t)=>e.time.localeCompare(t.time))),e}function J(){let e=document.getElementById(`call-cal-month`),t=document.getElementById(`call-cal-grid`);if(!e||!t)return;if(!K.year){let e=new Date;K.year=e.getFullYear(),K.month=e.getMonth()}e.textContent=`${V[K.month]} ${K.year}`;let n=q(),r=U(new Date);t.innerHTML=``;let i=(new Date(K.year,K.month,1).getDay()+6)%7,a=new Date(K.year,K.month+1,0).getDate();for(let e=0;e<i;e++){let e=document.createElement(`span`);e.className=`adm-callcal__cell adm-callcal__cell--empty`,t.appendChild(e)}for(let e=1;e<=a;e++){let i=`${K.year}-${String(K.month+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,a=(n[i]||[]).length,o=i<r,s=document.createElement(`button`);s.type=`button`,s.className=`adm-callcal__cell`,s.innerHTML=`<span>${e}</span>${a?`<span class="adm-callcal__count">${a}</span>`:``}`,a&&s.classList.add(`adm-callcal__cell--has`),o&&s.classList.add(`adm-callcal__cell--past`),i===K.selectedDay&&s.classList.add(`adm-callcal__cell--sel`),K.editing&&!o?(s.classList.add(`adm-callcal__cell--edit`),s.addEventListener(`click`,()=>{K.selectedDay=i,J(),Y()})):s.disabled=!0,t.appendChild(s)}}function Y(){let e=document.getElementById(`call-day-editor`),t=document.getElementById(`call-day-label`),n=document.getElementById(`call-day-slots`);if(!e||!t||!n)return;if(!K.editing||!K.selectedDay){e.classList.add(`is-hidden`);return}e.classList.remove(`is-hidden`),t.textContent=W(K.selectedDay);let r=q()[K.selectedDay]||[];r.length===0?n.innerHTML=`<span class="adm-dayeditor__empty">Aucun créneau. Ajoutez-en un ci-dessous.</span>`:n.innerHTML=r.map(e=>`
        <span class="adm-slotchip" data-slot-id="${e.id}">
          ${Z(e.time)}
          <button type="button" class="adm-slotchip__x" data-remove-slot="${e.id}" aria-label="Retirer">&times;</button>
        </span>`).join(``)}async function ge(){if(!K.selectedDay)return;let e=document.getElementById(`call-day-time`),t=e?.value;if(!t){alert(`Choisissez une heure.`);return}if((q()[K.selectedDay]||[]).some(e=>e.time===t)){alert(`Ce créneau existe déjà.`);return}let{data:n,error:r}=await i.from(`call_slots`).insert({date:K.selectedDay,time:t}).select().single();if(r||!n){alert(`Erreur lors de l'ajout. Réessayez.`);return}g.push(n),e&&(e.value=``),J(),Y()}async function _e(e){let t=g.find(t=>t.id===e);if(_.some(e=>e.status===`paid`&&t&&e.slot_date===t.date&&e.slot_time===t.time)&&!confirm(`Ce créneau a une réservation payée. Le retirer ne supprime pas la réservation. Continuer ?`))return;let{error:n}=await i.from(`call_slots`).delete().eq(`id`,e);if(n){alert(`Erreur lors de la suppression. Réessayez.`);return}g=g.filter(t=>t.id!==e),J(),Y()}document.getElementById(`call-day-add`)?.addEventListener(`click`,ge),document.getElementById(`call-day-slots`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-remove-slot]`);t&&_e(t.dataset.removeSlot)}),document.getElementById(`call-cal-prev`)?.addEventListener(`click`,()=>{K.month--,K.month<0&&(K.month=11,K.year--),J()}),document.getElementById(`call-cal-next`)?.addEventListener(`click`,()=>{K.month++,K.month>11&&(K.month=0,K.year++),J()}),document.getElementById(`call-cal-toggle`)?.addEventListener(`click`,()=>{K.editing=!K.editing;let e=document.getElementById(`call-cal-toggle`),t=document.getElementById(`call-cal-hint`);e.textContent=K.editing?`Fermer le calendrier`:`Ouvrir le calendrier`,e.classList.toggle(`adm-cal-toggle--on`,K.editing),t&&(t.textContent=K.editing?`Cliquez sur un jour pour ajouter ou retirer des créneaux horaires.`:`Cliquez sur « Ouvrir le calendrier » pour ajouter ou retirer vos créneaux disponibles.`),K.editing||(K.selectedDay=null),J(),Y()});function ve(){let e=document.getElementById(`call-bookings-list`),t=document.getElementById(`call-bookings-count`);if(!e)return;let n=_.filter(e=>e.status===`paid`).sort((e,t)=>(t.slot_date+t.slot_time).localeCompare(e.slot_date+e.slot_time));if(t&&(t.textContent=n.length?String(n.length):``),n.length===0){e.innerHTML=`<p class="adm-callbookings__empty">Aucun appel réservé pour le moment.</p>`;return}e.innerHTML=n.map(e=>{let t=[[`Date de l'appel`,W(e.slot_date)],[`Heure`,e.slot_time||`—`],[`Nom & prénom`,e.name||`—`],[`Nombre de personnes`,e.group_size?String(e.group_size):`—`],[`Date d'arrivée`,e.arrival?W(e.arrival):`—`],[`Date de départ`,e.departure?W(e.departure):`—`],[`Téléphone`,e.phone||`—`],[`Email`,e.email||`—`],[`Billets d'avion`,e.has_tickets?`Oui, déjà réservés`:`Non, pas encore`],[`Montant payé`,e.amount_paid?H(e.amount_paid):`—`]].map(([e,t])=>`
        ...
        <div class="adm-cb" data-cb>
          <button type="button" class="adm-cb__head" data-cb-toggle>
            ...
            <span class="adm-cb__meta">${Z(W(e.slot_date))} · ${Z(e.slot_time||``)}</span>
        ...
        </div>`}).join(``)}document.getElementById(`call-bookings-list`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-cb-toggle]`);t&&t.parentElement?.classList.toggle(`adm-cb--open`)});function ye(){me(),J(),Y(),ve()}var X=`pending`;function be(){return m}async function xe(e,t){let{error:n}=await i.from(`reviews`).update({status:t}).eq(`id`,e);if(n)return alert(`Erreur, réessayez.`),!1;let r=m.find(t=>t.id===e);return r&&(r.status=t),!0}function Z(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Q(){let e=be().filter(e=>e.status===`pending`),t=document.getElementById(`pending-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function Se(){let e=N().filter(e=>e.status===`pending-refusal`),t=document.getElementById(`refusal-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function $(){let e=be(),t=e.filter(e=>e.status===`pending`),n=e.filter(e=>e.status===`approved`),r=e.filter(e=>e.status===`rejected`),a=document.getElementById(`reviews-sub`);a&&(a.textContent=`${e.length} avis au total — ${t.length} en attente`);let o=document.getElementById(`rtab-pending-count`),s=document.getElementById(`rtab-approved-count`),c=document.getElementById(`rtab-rejected-count`);o&&(o.textContent=t.length>0?`(${t.length})`:``),s&&(s.textContent=n.length>0?`(${n.length})`:``),c&&(c.textContent=r.length>0?`(${r.length})`:``);let l=X===`pending`?t:X===`approved`?n:r,u=document.getElementById(`reviews-list`);if(u){if(l.length===0){u.innerHTML=`<div class="adm-reviews__empty">Aucun avis ${X===`pending`?`en attente`:X===`approved`?`approuvé`:`refusé`}.</div>`;return}u.innerHTML=l.map(e=>{let t=`★`.repeat(e.stars||5)+`☆`.repeat(5-(e.stars||5)),n=e.date?new Date(e.date).toLocaleDateString(`fr-FR`,{day:`numeric`,month:`long`,year:`numeric`}):``,r=``;return r=e.status===`pending`?`
      ...
      `}).join(``),u.querySelectorAll(`[data-rv-approve]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvApprove;await xe(t,`approved`)&&($(),Q())})}),u.querySelectorAll(`[data-rv-reject]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvReject;await xe(t,`rejected`)&&($(),Q())})}),u.querySelectorAll(`[data-rv-delete]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvDelete,{error:n}=await i.from(`reviews`).delete().eq(`id`,t);if(n){alert(`Erreur, réessayez.`);return}m=m.filter(e=>e.id!==t),$(),Q()})})}}document.querySelectorAll(`[data-rtab]`).forEach(e=>{e.addEventListener(`click`,()=>{X=e.dataset.rtab,document.querySelectorAll(`[data-rtab]`).forEach(e=>e.classList.remove(`adm-reviews__tab--active`)),e.classList.add(`adm-reviews__tab--active`),$()})}),Q(),Se();