import{a as e,c as t,d as n,i as r,l as i,n as a,o,r as s,s as c}from"./supabase.BampuV_k.js";var l=n(r,s,{auth:{persistSession:!1,autoRefreshToken:!1}}),u=document.getElementById(`login-gate`),d=document.getElementById(`dashboard`),f=[],p=[],m=[],h={},g=[],_=[],v=[];async function y(){let[e,n,r,a,s,l]=await Promise.all([i.from(`guides`).select(`*`).order(`created_at`),i.from(`bookings`).select(`*`).order(`date`),i.from(`reviews`).select(`*`).order(`created_at`,{ascending:!1}),i.from(`availability`).select(`guide_id, date`),i.from(`call_slots`).select(`*`).order(`date`),i.from(`call_bookings`).select(`*`).order(`slot_date`)]);f=(e.data||[]).map(c),p=(n.data||[]).map(o),v=n.data||[],m=(r.data||[]).map(t),h={},(a.data||[]).forEach(e=>{(h[e.guide_id]=h[e.guide_id]||[]).push(e.date)}),g=s.data||[],_=l.data||[]}async function b(){await y(),u.classList.add(`is-hidden`),d.classList.remove(`is-hidden`),ie(),Z(),Q(),we()}(async()=>{await e()&&await b()})(),document.getElementById(`login-form`).addEventListener(`submit`,async t=>{t.preventDefault();let n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pass`).value,a=document.getElementById(`login-error`);a.classList.add(`is-hidden`);let{error:o}=await i.auth.signInWithPassword({email:n,password:r});if(o||!await e()){await i.auth.signOut(),a.classList.remove(`is-hidden`);return}await b()}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await i.auth.signOut(),u.classList.remove(`is-hidden`),d.classList.add(`is-hidden`)});var x=document.getElementById(`modal-overlay`),S=document.getElementById(`photo-input`),C=document.getElementById(`photo-empty`),ee=document.getElementById(`photo-editor`),w=document.getElementById(`photo-result`),T=document.getElementById(`photo-crop-img`),E=document.getElementById(`photo-crop`),D=document.getElementById(`photo-zoom`),O=document.getElementById(`photo-result-circle`),k={x:0,y:0,scale:1,dragging:!1,startX:0,startY:0,origX:0,origY:0,imgSrc:``};function A(e){C.classList.toggle(`is-hidden`,e!==`empty`),ee.classList.toggle(`is-hidden`,e!==`editor`),w.classList.toggle(`is-hidden`,e!==`result`)}function j(){T.style.transform=`translate(${k.x}px, ${k.y}px) scale(${k.scale})`}function M(){let e=T.naturalWidth*k.scale,t=T.naturalHeight*k.scale,n=160-e,r=160-t;k.x=Math.min(0,Math.max(n,k.x)),k.y=Math.min(0,Math.max(r,k.y))}document.getElementById(`photo-btn`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-placeholder`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-change-btn`).addEventListener(`click`,()=>S.click()),S.addEventListener(`change`,()=>{let e=S.files?.[0];if(!e)return;let t=new FileReader;t.onload=e=>{k.imgSrc=e.target?.result,T.src=k.imgSrc,T.onload=()=>{k.scale=160/Math.min(T.naturalWidth,T.naturalHeight),k.x=(160-T.naturalWidth*k.scale)/2,k.y=(160-T.naturalHeight*k.scale)/2,D.min=String(Math.round(k.scale*100)),D.max=String(Math.round(k.scale*100*3)),D.value=String(Math.round(k.scale*100)),j(),A(`editor`)}},t.readAsDataURL(e)}),E.addEventListener(`mousedown`,e=>{e.preventDefault(),k.dragging=!0,k.startX=e.clientX,k.startY=e.clientY,k.origX=k.x,k.origY=k.y,E.style.cursor=`grabbing`}),window.addEventListener(`mousemove`,e=>{k.dragging&&(k.x=k.origX+(e.clientX-k.startX),k.y=k.origY+(e.clientY-k.startY),M(),j())}),window.addEventListener(`mouseup`,()=>{k.dragging=!1,E.style.cursor=`grab`}),E.addEventListener(`touchstart`,e=>{e.touches.length===1&&(k.dragging=!0,k.startX=e.touches[0].clientX,k.startY=e.touches[0].clientY,k.origX=k.x,k.origY=k.y)},{passive:!0}),window.addEventListener(`touchmove`,e=>{!k.dragging||e.touches.length!==1||(k.x=k.origX+(e.touches[0].clientX-k.startX),k.y=k.origY+(e.touches[0].clientY-k.startY),M(),j())},{passive:!0}),window.addEventListener(`touchend`,()=>{k.dragging=!1}),D.addEventListener(`input`,()=>{let e=k.scale;k.scale=parseInt(D.value)/100;let t=k.scale/e;k.x=80-(80-k.x)*t,k.y=80-(80-k.y)*t,M(),j()}),E.addEventListener(`wheel`,e=>{e.preventDefault();let t=parseInt(D.min)/100,n=parseInt(D.max)/100,r=e.deltaY>0?-.05:.05,i=k.scale;k.scale=Math.min(n,Math.max(t,k.scale+r));let a=k.scale/i;k.x=80-(80-k.x)*a,k.y=80-(80-k.y)*a,M(),D.value=String(Math.round(k.scale*100)),j()},{passive:!1}),document.getElementById(`photo-crop-cancel`).addEventListener(`click`,()=>{A(`empty`),S.value=``}),document.getElementById(`photo-crop-confirm`).addEventListener(`click`,()=>{let e=document.createElement(`canvas`);e.width=320,e.height=320,e.getContext(`2d`).drawImage(T,k.x*2,k.y*2,T.naturalWidth*k.scale*2,T.naturalHeight*k.scale*2);let t=e.toDataURL(`image/jpeg`,.9);O.style.backgroundImage=`url(${t})`,A(`result`)});function N(){return f}function P(){return p}function te(e){return h[e]||[]}function F(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var ne=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`],re=[`Lu`,`Ma`,`Me`,`Je`,`Ve`,`Sa`,`Di`];function I(e,t,n,r){let i=new Set(te(t)),a=new Set(P().filter(e=>e.guideId===t&&e.status!==`reassigned`).map(e=>e.date)),o=new Date(r,n,1).getDay(),s=o===0?6:o-1,c=new Date(r,n+1,0).getDate(),l=new Date,u=re.map(e=>`<span class="adm-cal__day-label">${e}</span>`).join(``);for(let e=0;e<s;e++)u+=`<span class="adm-cal__empty"></span>`;for(let e=1;e<=c;e++){let t=`${r}-${String(n+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,o=a.has(t),s=i.has(t),c=new Date(r,n,e)<new Date(l.getFullYear(),l.getMonth(),l.getDate()),d=`adm-cal__day`;c?d+=` adm-cal__day--past`:o?d+=` adm-cal__day--booked`:s?d+=` adm-cal__day--avail`:d+=` adm-cal__day--busy`,u+=`<span class="${d}">${e}</span>`}let d=e.querySelector(`.adm-cal__grid`),f=e.querySelector(`.adm-cal__title`);d&&(d.innerHTML=u),f&&(f.textContent=`${ne[n]} ${r}`);let p=e.querySelector(`[data-cal-nav]`);p&&(p.dataset.month=n.toString(),p.dataset.year=r.toString())}function ie(){let e=document.getElementById(`panel-guides`);e.innerHTML=``,N().forEach(t=>e.appendChild(L(t))),ae(),Q()}function L(e){let t=e.city===`medina`?`Medine`:`La Mecque`,n=(e.firstName?.[0]||``)+(e.lastName?.[0]||``),r=e.photo?`<div class="adm-g__avatar adm-g__avatar--photo" style="background-image:url(${e.photo})"></div>`:`<div class="adm-g__avatar adm-g__avatar--${e.city}">${F(n)}</div>`,a=P().filter(t=>t.guideId===e.id&&t.status!==`reassigned`),o=a.filter(e=>e.status===`pending-refusal`),s=o.length>0,c=a.length>0,l=document.createElement(`div`);l.className=`adm-g`,l.dataset.guideId=e.id;let u=``;u=s?`<div class="adm-g__notif">
    ...
    `,l.querySelector(`[data-open-creds="${e.id}"]`).addEventListener(`click`,()=>{document.getElementById(`creds-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelector(`[data-delete-guide="${e.id}"]`).addEventListener(`click`,async()=>{if(!confirm(`Supprimer ${e.firstName} ${e.lastName} ?`))return;let{error:t}=await i.from(`guides`).delete().eq(`id`,e.id);if(t){alert(`Erreur lors de la suppression. Réessayez.`);return}f=f.filter(t=>t.id!==e.id),l.remove(),ae()}),l.querySelector(`[data-edit-guide="${e.id}"]`).addEventListener(`click`,()=>{se(e.id)});let h=l.querySelector(`[data-toggle-bookings="${e.id}"]`);h&&h.addEventListener(`click`,()=>{document.getElementById(`bookings-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-bk-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-bk-row]`);if(!t)return;let n=t.querySelector(`.adm-g__bk-detail`),r=e.querySelector(`.adm-g__bk-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})});let g=l.querySelector(`[data-toggle-refusal="${e.id}"]`);return g&&g.addEventListener(`click`,()=>{document.getElementById(`refusal-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-rcard]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-rcard]`);if(!t)return;let n=t.querySelector(`.adm-g__rcard-body`),r=e.querySelector(`.adm-g__rcard-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-cal`).forEach(e=>{let t=e.querySelector(`[data-cal-nav]`);if(!t)return;let n=t.dataset.guideId;I(e,n,parseInt(t.dataset.month),parseInt(t.dataset.year));let r=e.querySelector(`[data-cal-prev]`),i=e.querySelector(`[data-cal-next]`);r&&r.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r--,r<0&&(r=11,i--),I(e,n,r,i)}),i&&i.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r++,r>11&&(r=0,i++),I(e,n,r,i)})}),l.querySelectorAll(`[data-toggle-ref-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-ref-row]`);if(!t)return;let n=t.querySelector(`.adm-g__ref-detail`),r=e.querySelector(`.adm-g__ref-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-g__rcard-radio`).forEach(e=>{e.addEventListener(`click`,e=>{e.stopPropagation()})}),l.querySelectorAll(`[data-confirm-refusal]`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t,r=n.dataset.bookingId,a=n.dataset.radioName,o=l.querySelector(`input[name="${a}"]:checked`),s=o?o.value:null,c=P().find(e=>e.id===r);if(!c)return;n.disabled=!0;let{error:u}=await i.from(`bookings`).update({status:`reassigned`}).eq(`id`,r);if(u){n.disabled=!1,alert(`Une erreur est survenue. Réessayez.`);return}if(s){let t=N().find(t=>t.id===e.id),{error:n}=await i.from(`bookings`).insert({guide_id:s,date:c.date,slot:c.slot||null,activity_slug:c.activitySlug||null,activity_name:c.activityName||null,client_name:c.clientName||null,client_email:c.clientEmail||null,client_phone:c.clientPhone||null,status:`confirmed`,replacement_of:t?`${t.firstName} ${t.lastName}`:null});n&&alert(`Le refus a été confirmé, mais la réassignation a échoué (date déjà prise ?).`)}await y(),ie()})}),l}function ae(){let e=document.querySelector(`.adm-top__sub`);if(e){let t=N().length;e.textContent=`${t} guide${t===1?``:`s`} enregistre${t===1?``:`s`}`}}var R=null,oe=document.getElementById(`modal-title`),z=document.getElementById(`add-guide-form`);function se(e){let t=N().find(t=>t.id===e);if(!t)return;R=e,oe.textContent=`Modifier le guide`,z.querySelector(`[name="prenom"]`).value=t.firstName||``,z.querySelector(`[name="nom"]`).value=t.lastName||``,z.querySelector(`[name="ville"]`).value=t.city||``,z.querySelector(`[name="gender"]`).value=t.gender||`homme`,z.querySelector(`[name="prix"]`).value=t.pricePerDay?String(t.pricePerDay):``,z.querySelector(`[name="service_prix"]`).value=t.servicePrice?String(t.servicePrice):``,z.querySelector(`[name="supplement_prix"]`).value=t.supplement710?String(t.supplement710):``,z.querySelector(`[name="diplome"]`).value=t.diploma||``,z.querySelector(`[name="langues"]`).value=(t.languages||[]).join(`, `),z.querySelector(`[name="description"]`).value=t.description||``,z.querySelectorAll(`[name="services"]`).forEach(e=>{e.checked=(t.services||[]).includes(e.value)});let n=z.querySelector(`[name="identifiant"]`),r=z.querySelector(`[name="mdp"]`);n.value=t.login||``,n.disabled=!0,r.value=``,r.placeholder=`Non modifiable ici`,r.disabled=!0,t.photo?(O.style.backgroundImage=`url(${t.photo})`,A(`result`)):A(`empty`),x.classList.remove(`is-hidden`)}function B(){R=null,oe.textContent=`Ajouter un guide`,z.reset();let e=z.querySelector(`[name="identifiant"]`),t=z.querySelector(`[name="mdp"]`);e.disabled=!1,t.disabled=!1,t.placeholder=`mot de passe (6 caractères min.)`,A(`empty`),O.style.backgroundImage=``}document.getElementById(`open-add-guide`).addEventListener(`click`,()=>{B(),x.classList.remove(`is-hidden`)}),document.getElementById(`close-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),B()}),document.getElementById(`cancel-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),B()}),x.addEventListener(`click`,e=>{e.target===x&&(x.classList.add(`is-hidden`),B())}),z.addEventListener(`submit`,async e=>{e.preventDefault();let t=z,n=t.querySelector(`[name="prenom"]`).value.trim(),r=t.querySelector(`[name="nom"]`).value.trim(),o=t.querySelector(`[name="ville"]`).value,s=t.querySelector(`[name="gender"]`).value,u=t.querySelector(`[name="prix"]`).value,d=t.querySelector(`[name="service_prix"]`).value,p=t.querySelector(`[name="supplement_prix"]`).value,m=t.querySelector(`[name="diplome"]`).value.trim(),h=t.querySelector(`[name="langues"]`).value.trim(),g=t.querySelector(`[name="description"]`).value.trim(),_=t.querySelector(`[name="identifiant"]`).value.trim().toLowerCase(),v=t.querySelector(`[name="mdp"]`).value;if(!n||!r)return;let y=t.querySelector(`.adm-modal__submit`);y.disabled=!0,y.textContent=`Enregistrement...`;let b=()=>{y.disabled=!1,y.textContent=`Enregistrer`},S=O.style.backgroundImage.match(/url\("?([^"]*)"?\)/),C=S?S[1]:``,ee=Array.from(t.querySelectorAll(`[name="services"]:checked`)).map(e=>e.value),w={first_name:n,last_name:r,city:o||`medina`,gender:s||`homme`,diploma:m||null,description:g||null,languages:h?h.split(`,`).map(e=>e.trim()).filter(Boolean):[],price_per_day:u?parseInt(u):0,service_price:d?parseInt(d):0,supplement_7_10:p?parseInt(p):0,services:ee};if(R){let e=N().find(e=>e.id===R);if(!e){b();return}!C&&e.photo&&(C=e.photo);let{data:t,error:n}=await i.from(`guides`).update({...w,photo:C||null}).eq(`id`,R).select().single();if(b(),n||!t){alert(`Erreur lors de la mise à jour. Réessayez.`);return}let r=c(t),a=f.findIndex(e=>e.id===R);a!==-1&&(f[a]=r);let o=document.querySelector(`[data-guide-id="${R}"]`);if(o){let e=L(r);o.replaceWith(e),e.scrollIntoView({behavior:`smooth`,block:`center`})}}else{if(!_||!/^[a-z0-9._-]+$/.test(_)){b(),alert(`Identifiant invalide : lettres minuscules, chiffres, points, tirets uniquement (sans espaces ni accents).`);return}if(!v||v.length<6){b(),alert(`Le mot de passe doit contenir au moins 6 caractères.`);return}let{data:e,error:t}=await l.auth.signUp({email:_+a,password:v});if(t||!e.user){b(),alert(`Impossible de créer le compte du guide : ${t?.message||`erreur inconnue`}.\n(Identifiant déjà utilisé ?)`);return}let{data:n,error:r}=await i.from(`guides`).insert({...w,photo:C||null,login:_,auth_user_id:e.user.id}).select().single();if(b(),r||!n){alert(`Le compte a été créé mais l'enregistrement du guide a échoué. Réessayez.`);return}let o=c(n);f.push(o);let s=document.getElementById(`panel-guides`),u=L(o);s.appendChild(u),u.scrollIntoView({behavior:`smooth`,block:`center`})}ae(),x.classList.add(`is-hidden`),B()});var ce=document.querySelectorAll(`[data-nav-tab]`),le=document.getElementById(`panel-home`),ue=document.getElementById(`panel-guides-header`),de=document.getElementById(`panel-guides`),fe=document.getElementById(`panel-reviews`);ce.forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navTab;ce.forEach(e=>e.classList.remove(`adm-side__link--active`)),e.classList.add(`adm-side__link--active`),le.classList.add(`is-hidden`),ue.classList.add(`is-hidden`),de.classList.add(`is-hidden`),fe.classList.add(`is-hidden`),n===`home`?(le.classList.remove(`is-hidden`),we(),Te()):n===`reviews`?(fe.classList.remove(`is-hidden`),$()):(ue.classList.remove(`is-hidden`),de.classList.remove(`is-hidden`))})});var V=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`];function H(e){return(Math.round(e)/100).toLocaleString(`fr-FR`,{minimumFractionDigits:0,maximumFractionDigits:2})+` €`}function U(e){return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`}function W(e){let[t,n,r]=e.split(`-`).map(Number);return`${r} ${V[n-1].toLowerCase()} ${t}`}function pe(){let e=[];return v.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),_.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),e}function me(){let e=pe(),t=new Date,n=U(t),r=new Date(t),i=(t.getDay()+6)%7;r.setDate(t.getDate()-i);let a=U(r),o=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-01`,s=0,c=0,l=0,u=0;e.forEach(e=>{u+=e.amount,e.date===n&&(s+=e.amount),e.date>=a&&(c+=e.amount),e.date>=o&&(l+=e.amount)});let d=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=H(t))};d(`stat-today`,s),d(`stat-week`,c),d(`stat-month`,l),d(`stat-total`,u),he()}var G=`day`;function he(){let e=document.getElementById(`stat-breakdown`);if(!e)return;let t=pe(),n={};t.forEach(e=>{if(!e.date)return;let[t,r,i]=e.date.split(`-`).map(Number),a=``,o=``,s=``;if(G===`day`)a=e.date,o=W(e.date),s=e.date;else if(G===`month`)a=`${t}-${String(r).padStart(2,`0`)}`,o=`${V[r-1]} ${t}`,s=a;else{let e=new Date(t,r-1,i),n=(e.getDay()+6)%7;e.setDate(e.getDate()-n);let c=U(e);a=c,o=`Semaine du ${W(c)}`,s=c}n[a]||(n[a]={label:o,sort:s,amount:0}),n[a].amount+=e.amount});let r=Object.values(n).sort((e,t)=>t.sort.localeCompare(e.sort));if(r.length===0){e.innerHTML=`<p class="adm-breakdown__empty">Aucun encaissement pour le moment.</p>`;return}e.innerHTML=r.map(e=>`
      <div class="adm-breakdown__row">
      ...
      </div>`).join(``)}document.querySelectorAll(`[data-stat-period]`).forEach(e=>{e.addEventListener(`click`,()=>{G=e.dataset.statPeriod,document.querySelectorAll(`[data-stat-period]`).forEach(e=>e.classList.remove(`adm-seg__btn--active`)),e.classList.add(`adm-seg__btn--active`),he()})});var ge=`Le matin`,K={year:0,month:0,editing:!1},q=!1;function _e(){let e=new Set;return g.forEach(t=>e.add(t.date)),e}function ve(e){let t=g.find(t=>t.date===e);return t?t.id:null}function J(){let e=document.getElementById(`call-cal-month`),t=document.getElementById(`call-cal-grid`);if(!e||!t)return;if(!K.year){let e=new Date;K.year=e.getFullYear(),K.month=e.getMonth()}e.textContent=`${V[K.month]} ${K.year}`;let n=_e(),r=U(new Date);t.innerHTML=``;let i=(new Date(K.year,K.month,1).getDay()+6)%7,a=new Date(K.year,K.month+1,0).getDate();for(let e=0;e<i;e++){let e=document.createElement(`span`);e.className=`adm-callcal__cell adm-callcal__cell--empty`,t.appendChild(e)}for(let e=1;e<=a;e++){let i=`${K.year}-${String(K.month+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,a=n.has(i),o=i<r,s=document.createElement(`button`);s.type=`button`,s.className=`adm-callcal__cell`,s.innerHTML=`<span>${e}</span>`,a&&s.classList.add(`adm-callcal__cell--has`),o&&s.classList.add(`adm-callcal__cell--past`),K.editing&&!o?(s.classList.add(`adm-callcal__cell--edit`),s.addEventListener(`click`,()=>ye(i))):s.disabled=!0,t.appendChild(s)}}async function ye(e){if(q)return;q=!0;let t=ve(e);if(t){if(_.some(t=>t.status===`paid`&&t.slot_date===e)&&!confirm(`Ce jour a une réservation payée. Le fermer ne supprime pas la réservation. Continuer ?`)){q=!1;return}let{error:n}=await i.from(`call_slots`).delete().eq(`id`,t);n?alert(`Erreur. Réessayez.`):g=g.filter(e=>e.id!==t)}else{let{data:t,error:n}=await i.from(`call_slots`).insert({date:e,time:ge}).select().single();!n&&t?g.push(t):alert(`Erreur. Réessayez.`)}q=!1,J()}async function be(){if(q)return;q=!0;let e=U(new Date),t=_e(),n=new Date(K.year,K.month+1,0).getDate(),r=[];for(let i=1;i<=n;i++){let n=`${K.year}-${String(K.month+1).padStart(2,`0`)}-${String(i).padStart(2,`0`)}`;n>=e&&!t.has(n)&&r.push({date:n,time:ge})}if(r.length){let{data:e,error:t}=await i.from(`call_slots`).insert(r).select();!t&&e?g.push(...e):alert(`Erreur. Réessayez.`)}q=!1,J()}async function xe(){if(q)return;let e=`${K.year}-${String(K.month+1).padStart(2,`0`)}-`,t=g.filter(t=>t.date.startsWith(e));if(t.length===0||_.some(t=>t.status===`paid`&&t.slot_date.startsWith(e))&&!confirm(`Certains jours ont des réservations payées. Les fermer ne supprime pas les réservations. Continuer ?`)||!confirm(`Fermer tous les jours de ce mois ?`))return;q=!0;let n=t.map(e=>e.id),{error:r}=await i.from(`call_slots`).delete().in(`id`,n);r?alert(`Erreur. Réessayez.`):g=g.filter(t=>!t.date.startsWith(e)),q=!1,J()}document.getElementById(`call-cal-selectall`)?.addEventListener(`click`,be),document.getElementById(`call-cal-clearall`)?.addEventListener(`click`,xe),document.getElementById(`call-cal-prev`)?.addEventListener(`click`,()=>{K.month--,K.month<0&&(K.month=11,K.year--),J()}),document.getElementById(`call-cal-next`)?.addEventListener(`click`,()=>{K.month++,K.month>11&&(K.month=0,K.year++),J()});var Se=!1;document.getElementById(`call-cal-show`)?.addEventListener(`click`,()=>{Se=!Se;let e=document.getElementById(`call-cal-show`),t=document.getElementById(`call-cal-edit`),n=document.getElementById(`call-cal-container`),r=document.getElementById(`call-cal-hint`),i=document.getElementById(`call-cal-monthactions`);Se?(n.classList.remove(`is-hidden`),t.classList.remove(`is-hidden`),e.textContent=`Masquer`,J()):(K.editing=!1,n.classList.add(`is-hidden`),t.classList.add(`is-hidden`),t.textContent=`Modifier`,t.classList.remove(`adm-cal-toggle--on`),r?.classList.add(`is-hidden`),i?.classList.add(`is-hidden`),e.textContent=`Voir le calendrier`)}),document.getElementById(`call-cal-edit`)?.addEventListener(`click`,()=>{K.editing=!K.editing;let e=document.getElementById(`call-cal-edit`),t=document.getElementById(`call-cal-hint`),n=document.getElementById(`call-cal-monthactions`);e.textContent=K.editing?`Terminer`:`Modifier`,e.classList.toggle(`adm-cal-toggle--on`,K.editing),t&&(t.textContent=`Cliquez sur un jour pour l'ouvrir ou le fermer à la réservation (appel le matin).`,t.classList.toggle(`is-hidden`,!K.editing)),n?.classList.toggle(`is-hidden`,!K.editing),J()});function Ce(){let e=document.getElementById(`call-bookings-list`),t=document.getElementById(`call-bookings-count`);if(!e)return;let n=_.filter(e=>e.status===`paid`).sort((e,t)=>(t.slot_date+t.slot_time).localeCompare(e.slot_date+e.slot_time));if(t&&(t.textContent=n.length?String(n.length):``),n.length===0){e.innerHTML=`<p class="adm-callbookings__empty">Aucun appel réservé pour le moment.</p>`;return}e.innerHTML=n.map(e=>{let t=[[`Date de l'appel`,W(e.slot_date)],[`Heure`,e.slot_time||`—`],[`Nom & prénom`,e.name||`—`],[`Nombre de personnes`,e.group_size?String(e.group_size):`—`],[`Date d'arrivée`,e.arrival?W(e.arrival):`—`],[`Date de départ`,e.departure?W(e.departure):`—`],[`Téléphone`,e.phone||`—`],[`Email`,e.email||`—`],[`Billets d'avion`,e.has_tickets?`Oui, déjà réservés`:`Non, pas encore`],[`Montant payé`,e.amount_paid?H(e.amount_paid):`—`]].map(([e,t])=>`
        ...
        <div class="adm-cb" data-cb>
          <button type="button" class="adm-cb__head" data-cb-toggle>
            ...
            <span class="adm-cb__meta">${X(W(e.slot_date))} · ${X(e.slot_time||``)}</span>
        ...
        </div>`}).join(``)}document.getElementById(`call-bookings-list`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-cb-toggle]`);t&&t.parentElement?.classList.toggle(`adm-cb--open`)});function we(){me(),J(),Ce()}async function Te(){let[e,t,n]=await Promise.all([i.from(`call_slots`).select(`*`).order(`date`),i.from(`call_bookings`).select(`*`).order(`slot_date`),i.from(`bookings`).select(`*`).order(`date`)]);e.data&&(g=e.data),t.data&&(_=t.data),n.data&&(v=n.data,p=n.data.map(o)),we()}document.getElementById(`home-refresh`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`home-refresh`);e.disabled=!0;let t=e.innerHTML;e.textContent=`Actualisation…`,await Te(),e.disabled=!1,e.innerHTML=t});var Y=`pending`;function Ee(){return m}async function De(e,t){let{error:n}=await i.from(`reviews`).update({status:t}).eq(`id`,e);if(n)return alert(`Erreur, réessayez.`),!1;let r=m.find(t=>t.id===e);return r&&(r.status=t),!0}function X(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Z(){let e=Ee().filter(e=>e.status===`pending`),t=document.getElementById(`pending-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function Q(){let e=P().filter(e=>e.status===`pending-refusal`),t=document.getElementById(`refusal-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function $(){let e=Ee(),t=e.filter(e=>e.status===`pending`),n=e.filter(e=>e.status===`approved`),r=e.filter(e=>e.status===`rejected`),a=document.getElementById(`reviews-sub`);a&&(a.textContent=`${e.length} avis au total — ${t.length} en attente`);let o=document.getElementById(`rtab-pending-count`),s=document.getElementById(`rtab-approved-count`),c=document.getElementById(`rtab-rejected-count`);o&&(o.textContent=t.length>0?`(${t.length})`:``),s&&(s.textContent=n.length>0?`(${n.length})`:``),c&&(c.textContent=r.length>0?`(${r.length})`:``);let l=Y===`pending`?t:Y===`approved`?n:r,u=document.getElementById(`reviews-list`);if(u){if(l.length===0){u.innerHTML=`<div class="adm-reviews__empty">Aucun avis ${Y===`pending`?`en attente`:Y===`approved`?`approuvé`:`refusé`}.</div>`;return}u.innerHTML=l.map(e=>{let t=`★`.repeat(e.stars||5)+`☆`.repeat(5-(e.stars||5)),n=e.date?new Date(e.date).toLocaleDateString(`fr-FR`,{day:`numeric`,month:`long`,year:`numeric`}):``,r=``;return r=e.status===`pending`?`
      ...
      `}).join(``),u.querySelectorAll(`[data-rv-approve]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvApprove;await De(t,`approved`)&&($(),Z())})}),u.querySelectorAll(`[data-rv-reject]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvReject;await De(t,`rejected`)&&($(),Z())})}),u.querySelectorAll(`[data-rv-delete]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvDelete,{error:n}=await i.from(`reviews`).delete().eq(`id`,t);if(n){alert(`Erreur, réessayez.`);return}m=m.filter(e=>e.id!==t),$(),Z()})})}}document.querySelectorAll(`[data-rtab]`).forEach(e=>{e.addEventListener(`click`,()=>{Y=e.dataset.rtab,document.querySelectorAll(`[data-rtab]`).forEach(e=>e.classList.remove(`adm-reviews__tab--active`)),e.classList.add(`adm-reviews__tab--active`),$()})}),Z(),Q();