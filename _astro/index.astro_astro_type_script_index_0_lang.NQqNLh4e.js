import{a as e,c as t,d as n,i as r,l as i,n as a,o,r as s,s as c}from"./supabase.DVliaQv5.js";var l=n(r,s,{auth:{persistSession:!1,autoRefreshToken:!1}}),u=document.getElementById(`login-gate`),d=document.getElementById(`dashboard`),f=[],p=[],m=[],h={},g=[],_=[],v=[];async function y(){let[e,n,r,a,s,l]=await Promise.all([i.from(`guides`).select(`*`).order(`created_at`),i.from(`bookings`).select(`*`).order(`date`),i.from(`reviews`).select(`*`).order(`created_at`,{ascending:!1}),i.from(`availability`).select(`guide_id, date`),i.from(`call_slots`).select(`*`).order(`date`),i.from(`call_bookings`).select(`*`).order(`slot_date`)]);f=(e.data||[]).map(c),p=(n.data||[]).map(o),v=n.data||[],m=(r.data||[]).map(t),h={},(a.data||[]).forEach(e=>{(h[e.guide_id]=h[e.guide_id]||[]).push(e.date)}),g=s.data||[],_=l.data||[]}async function b(){await y(),u.classList.add(`is-hidden`),d.classList.remove(`is-hidden`),ae(),Q(),Ne(),ke()}(async()=>{await e()&&await b()})(),document.getElementById(`login-form`).addEventListener(`submit`,async t=>{t.preventDefault();let n=document.getElementById(`login-email`).value.trim(),r=document.getElementById(`login-pass`).value,a=document.getElementById(`login-error`);a.classList.add(`is-hidden`);let{error:o}=await i.auth.signInWithPassword({email:n,password:r});if(o||!await e()){await i.auth.signOut(),a.classList.remove(`is-hidden`);return}await b()}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await i.auth.signOut(),u.classList.remove(`is-hidden`),d.classList.add(`is-hidden`)});var x=document.getElementById(`modal-overlay`),S=document.getElementById(`photo-input`),C=document.getElementById(`photo-empty`),ee=document.getElementById(`photo-editor`),te=document.getElementById(`photo-result`),w=document.getElementById(`photo-crop-img`),T=document.getElementById(`photo-crop`),E=document.getElementById(`photo-zoom`),D=document.getElementById(`photo-result-circle`),O={x:0,y:0,scale:1,dragging:!1,startX:0,startY:0,origX:0,origY:0,imgSrc:``};function k(e){C.classList.toggle(`is-hidden`,e!==`empty`),ee.classList.toggle(`is-hidden`,e!==`editor`),te.classList.toggle(`is-hidden`,e!==`result`)}function A(){w.style.transform=`translate(${O.x}px, ${O.y}px) scale(${O.scale})`}function j(){let e=w.naturalWidth*O.scale,t=w.naturalHeight*O.scale,n=160-e,r=160-t;O.x=Math.min(0,Math.max(n,O.x)),O.y=Math.min(0,Math.max(r,O.y))}document.getElementById(`photo-btn`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-placeholder`).addEventListener(`click`,()=>S.click()),document.getElementById(`photo-change-btn`).addEventListener(`click`,()=>S.click()),S.addEventListener(`change`,()=>{let e=S.files?.[0];if(!e)return;let t=new FileReader;t.onload=e=>{O.imgSrc=e.target?.result,w.src=O.imgSrc,w.onload=()=>{O.scale=160/Math.min(w.naturalWidth,w.naturalHeight),O.x=(160-w.naturalWidth*O.scale)/2,O.y=(160-w.naturalHeight*O.scale)/2,E.min=String(Math.round(O.scale*100)),E.max=String(Math.round(O.scale*100*3)),E.value=String(Math.round(O.scale*100)),A(),k(`editor`)}},t.readAsDataURL(e)}),T.addEventListener(`mousedown`,e=>{e.preventDefault(),O.dragging=!0,O.startX=e.clientX,O.startY=e.clientY,O.origX=O.x,O.origY=O.y,T.style.cursor=`grabbing`}),window.addEventListener(`mousemove`,e=>{O.dragging&&(O.x=O.origX+(e.clientX-O.startX),O.y=O.origY+(e.clientY-O.startY),j(),A())}),window.addEventListener(`mouseup`,()=>{O.dragging=!1,T.style.cursor=`grab`}),T.addEventListener(`touchstart`,e=>{e.touches.length===1&&(O.dragging=!0,O.startX=e.touches[0].clientX,O.startY=e.touches[0].clientY,O.origX=O.x,O.origY=O.y)},{passive:!0}),window.addEventListener(`touchmove`,e=>{!O.dragging||e.touches.length!==1||(O.x=O.origX+(e.touches[0].clientX-O.startX),O.y=O.origY+(e.touches[0].clientY-O.startY),j(),A())},{passive:!0}),window.addEventListener(`touchend`,()=>{O.dragging=!1}),E.addEventListener(`input`,()=>{let e=O.scale;O.scale=parseInt(E.value)/100;let t=O.scale/e;O.x=80-(80-O.x)*t,O.y=80-(80-O.y)*t,j(),A()}),T.addEventListener(`wheel`,e=>{e.preventDefault();let t=parseInt(E.min)/100,n=parseInt(E.max)/100,r=e.deltaY>0?-.05:.05,i=O.scale;O.scale=Math.min(n,Math.max(t,O.scale+r));let a=O.scale/i;O.x=80-(80-O.x)*a,O.y=80-(80-O.y)*a,j(),E.value=String(Math.round(O.scale*100)),A()},{passive:!1}),document.getElementById(`photo-crop-cancel`).addEventListener(`click`,()=>{k(`empty`),S.value=``}),document.getElementById(`photo-crop-confirm`).addEventListener(`click`,()=>{let e=document.createElement(`canvas`);e.width=320,e.height=320,e.getContext(`2d`).drawImage(w,O.x*2,O.y*2,w.naturalWidth*O.scale*2,w.naturalHeight*O.scale*2);let t=e.toDataURL(`image/jpeg`,.9);D.style.backgroundImage=`url(${t})`,k(`result`)});function M(){return f}function N(){return p}function ne(e){return h[e]||[]}function P(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var re=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`],ie=[`Lu`,`Ma`,`Me`,`Je`,`Ve`,`Sa`,`Di`];function F(e,t,n,r){let i=new Set(ne(t)),a=new Set(N().filter(e=>e.guideId===t&&e.status!==`reassigned`).map(e=>e.date)),o=new Date(r,n,1).getDay(),s=o===0?6:o-1,c=new Date(r,n+1,0).getDate(),l=new Date,u=ie.map(e=>`<span class="adm-cal__day-label">${e}</span>`).join(``);for(let e=0;e<s;e++)u+=`<span class="adm-cal__empty"></span>`;for(let e=1;e<=c;e++){let t=`${r}-${String(n+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,o=a.has(t),s=i.has(t),c=new Date(r,n,e)<new Date(l.getFullYear(),l.getMonth(),l.getDate()),d=`adm-cal__day`;c?d+=` adm-cal__day--past`:o?d+=` adm-cal__day--booked`:s?d+=` adm-cal__day--avail`:d+=` adm-cal__day--busy`,u+=`<span class="${d}">${e}</span>`}let d=e.querySelector(`.adm-cal__grid`),f=e.querySelector(`.adm-cal__title`);d&&(d.innerHTML=u),f&&(f.textContent=`${re[n]} ${r}`);let p=e.querySelector(`[data-cal-nav]`);p&&(p.dataset.month=n.toString(),p.dataset.year=r.toString())}function ae(){let e=document.getElementById(`panel-guides`);e.innerHTML=``,M().forEach(t=>e.appendChild(I(t))),oe(),Ne()}function I(e){let t=e.city===`medina`?`Medine`:`La Mecque`,n=(e.firstName?.[0]||``)+(e.lastName?.[0]||``),r=e.photo?`<div class="adm-g__avatar adm-g__avatar--photo" style="background-image:url(${e.photo})"></div>`:`<div class="adm-g__avatar adm-g__avatar--${e.city}">${P(n)}</div>`,a=N().filter(t=>t.guideId===e.id&&t.status!==`reassigned`),o=a.filter(e=>e.status===`pending-refusal`),s=o.length>0,c=a.length>0,l=document.createElement(`div`);l.className=`adm-g`,l.dataset.guideId=e.id;let u=``;u=s?`<div class="adm-g__notif">
                ...
                <div class="adm-g__rcard-body is-hidden">
                  ...
                  <div class="adm-cal">
                    <div class="adm-cal__header">
                      <span class="adm-cal__title">${re[r]} ${i}</span>
    ...
    `,l.querySelector(`[data-open-creds="${e.id}"]`).addEventListener(`click`,()=>{document.getElementById(`creds-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelector(`[data-delete-guide="${e.id}"]`).addEventListener(`click`,async()=>{if(!confirm(`Supprimer ${e.firstName} ${e.lastName} ?`))return;let{error:t}=await i.from(`guides`).delete().eq(`id`,e.id);if(t){alert(`Erreur lors de la suppression. Réessayez.`);return}f=f.filter(t=>t.id!==e.id),l.remove(),oe()}),l.querySelector(`[data-edit-guide="${e.id}"]`).addEventListener(`click`,()=>{fe(e.id)});let h=l.querySelector(`[data-toggle-bookings="${e.id}"]`);h&&h.addEventListener(`click`,()=>{document.getElementById(`bookings-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-bk-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-bk-row]`);if(!t)return;let n=t.querySelector(`.adm-g__bk-detail`),r=e.querySelector(`.adm-g__bk-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})});let g=l.querySelector(`[data-toggle-refusal="${e.id}"]`);return g&&g.addEventListener(`click`,()=>{document.getElementById(`refusal-${e.id}`)?.classList.toggle(`is-hidden`)}),l.querySelectorAll(`[data-toggle-rcard]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-rcard]`);if(!t)return;let n=t.querySelector(`.adm-g__rcard-body`),r=e.querySelector(`.adm-g__rcard-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-cal`).forEach(e=>{let t=e.querySelector(`[data-cal-nav]`);if(!t)return;let n=t.dataset.guideId;F(e,n,parseInt(t.dataset.month),parseInt(t.dataset.year));let r=e.querySelector(`[data-cal-prev]`),i=e.querySelector(`[data-cal-next]`);r&&r.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r--,r<0&&(r=11,i--),F(e,n,r,i)}),i&&i.addEventListener(`click`,()=>{let r=parseInt(t.dataset.month),i=parseInt(t.dataset.year);r++,r>11&&(r=0,i++),F(e,n,r,i)})}),l.querySelectorAll(`[data-toggle-ref-detail]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.closest(`[data-ref-row]`);if(!t)return;let n=t.querySelector(`.adm-g__ref-detail`),r=e.querySelector(`.adm-g__ref-arrow`);if(n){let e=!n.classList.contains(`is-hidden`);n.classList.toggle(`is-hidden`),r&&(r.style.transform=e?``:`rotate(180deg)`)}})}),l.querySelectorAll(`.adm-g__rcard-radio`).forEach(e=>{e.addEventListener(`click`,e=>{e.stopPropagation()})}),l.querySelectorAll(`[data-confirm-refusal]`).forEach(t=>{t.addEventListener(`click`,async()=>{let n=t,r=n.dataset.bookingId,a=n.dataset.radioName,o=l.querySelector(`input[name="${a}"]:checked`),s=o?o.value:null,c=N().find(e=>e.id===r);if(!c)return;n.disabled=!0;let{error:u}=await i.from(`bookings`).update({status:`reassigned`}).eq(`id`,r);if(u){n.disabled=!1,alert(`Une erreur est survenue. Réessayez.`);return}if(s){let t=M().find(t=>t.id===e.id),{error:n}=await i.from(`bookings`).insert({guide_id:s,date:c.date,slot:c.slot||null,activity_slug:c.activitySlug||null,activity_name:c.activityName||null,client_name:c.clientName||null,client_email:c.clientEmail||null,client_phone:c.clientPhone||null,status:`confirmed`,replacement_of:t?`${t.firstName} ${t.lastName}`:null});n&&alert(`Le refus a été confirmé, mais la réassignation a échoué (date déjà prise ?).`)}await y(),ae()})}),l}function oe(){let e=document.querySelector(`.adm-top__sub`);if(e){let t=M().length;e.textContent=`${t} guide${t===1?``:`s`} enregistre${t===1?``:`s`}`}}var L=null,se=document.getElementById(`modal-title`),R=document.getElementById(`add-guide-form`),ce=[{slug:`omra-classique`,label:`Omra Privé Premium`},{slug:`guide-pmr-omra`,label:`Omra Privé Premium PMR`},{slug:`omra-masjid-aicha`,label:`Seconde Omra vers Masjid Aïcha`},{slug:`visite-historique-makkah`,label:`Visite historique de Makkah`},{slug:`visite-historique-medine`,label:`Visite historique de Médine`},{slug:`visite-masjid-nabawi`,label:`Visite du Masjid An-Nabawi`},{slug:`shopping-guide-medine`,label:`Orientation & Shopping Privilège (Femmes)`}],z=document.getElementById(`act-prices-list`);function le(e){return`<option value="">— Choisir une activité —</option>`+ce.map(t=>`<option value="${t.slug}"${t.slug===e?` selected`:``}>${Z(t.label)}</option>`).join(``)}function B(e=``,t=``){let n=document.createElement(`div`);n.className=`adm-actprice`,n.innerHTML=`
      ...
      <button type="button" class="adm-actprice__remove" aria-label="Retirer">&times;</button>`,n.querySelector(`.adm-actprice__remove`).addEventListener(`click`,()=>n.remove()),z.appendChild(n)}function ue(){let e={};return z.querySelectorAll(`.adm-actprice`).forEach(t=>{let n=t.querySelector(`.adm-actprice__act`).value,r=parseInt(t.querySelector(`.adm-actprice__price`).value);n&&r>0&&(e[n]=r)}),e}function de(e){z.innerHTML=``;let t=Object.entries(e||{});if(t.length===0){B();return}t.forEach(([e,t])=>B(e,String(t)))}document.getElementById(`act-prices-add`)?.addEventListener(`click`,()=>B());function fe(e){let t=M().find(t=>t.id===e);if(!t)return;L=e,se.textContent=`Modifier le guide`,R.querySelector(`[name="prenom"]`).value=t.firstName||``,R.querySelector(`[name="nom"]`).value=t.lastName||``,R.querySelector(`[name="ville"]`).value=t.city||``,R.querySelector(`[name="gender"]`).value=t.gender||`homme`,R.querySelector(`[name="service_prix"]`).value=t.servicePrice?String(t.servicePrice):``,R.querySelector(`[name="supplement_prix"]`).value=t.supplement710?String(t.supplement710):``,R.querySelector(`[name="diplome"]`).value=t.diploma||``,R.querySelector(`[name="langues"]`).value=(t.languages||[]).join(`, `),R.querySelector(`[name="description"]`).value=t.description||``;let n=t.activityPrices&&Object.keys(t.activityPrices).length?{...t.activityPrices}:{};Object.keys(n).length===0&&(t.services||[]).length&&(t.services||[]).forEach(e=>{n[e]=t.pricePerDay||0}),de(n);let r=R.querySelector(`[name="identifiant"]`),i=R.querySelector(`[name="mdp"]`);r.value=t.login||``,r.disabled=!0,i.value=``,i.placeholder=`Non modifiable ici`,i.disabled=!0,t.photo?(D.style.backgroundImage=`url(${t.photo})`,k(`result`)):k(`empty`),x.classList.remove(`is-hidden`)}function V(){L=null,se.textContent=`Ajouter un guide`,R.reset(),de({});let e=R.querySelector(`[name="identifiant"]`),t=R.querySelector(`[name="mdp"]`);e.disabled=!1,t.disabled=!1,t.placeholder=`mot de passe (6 caractères min.)`,k(`empty`),D.style.backgroundImage=``}document.getElementById(`open-add-guide`).addEventListener(`click`,()=>{V(),x.classList.remove(`is-hidden`)}),document.getElementById(`close-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),V()}),document.getElementById(`cancel-modal`).addEventListener(`click`,()=>{x.classList.add(`is-hidden`),V()}),x.addEventListener(`click`,e=>{e.target===x&&(x.classList.add(`is-hidden`),V())}),R.addEventListener(`submit`,async e=>{e.preventDefault();let t=R,n=t.querySelector(`[name="prenom"]`).value.trim(),r=t.querySelector(`[name="nom"]`).value.trim(),o=t.querySelector(`[name="ville"]`).value,s=t.querySelector(`[name="gender"]`).value,u=t.querySelector(`[name="service_prix"]`).value,d=t.querySelector(`[name="supplement_prix"]`).value,p=t.querySelector(`[name="diplome"]`).value.trim(),m=t.querySelector(`[name="langues"]`).value.trim(),h=t.querySelector(`[name="description"]`).value.trim(),g=t.querySelector(`[name="identifiant"]`).value.trim().toLowerCase(),_=t.querySelector(`[name="mdp"]`).value;if(!n||!r)return;let v=t.querySelector(`.adm-modal__submit`);v.disabled=!0,v.textContent=`Enregistrement...`;let y=()=>{v.disabled=!1,v.textContent=`Enregistrer`},b=D.style.backgroundImage.match(/url\("?([^"]*)"?\)/),S=b?b[1]:``,C=ue(),ee=Object.keys(C),te=Object.values(C),w=te.length?Math.min(...te):0,T={first_name:n,last_name:r,city:o||`medina`,gender:s||`homme`,diploma:p||null,description:h||null,languages:m?m.split(`,`).map(e=>e.trim()).filter(Boolean):[],price_per_day:w,service_price:u?parseInt(u):0,supplement_7_10:d?parseInt(d):0,services:ee,activity_prices:C};if(L){let e=M().find(e=>e.id===L);if(!e){y();return}!S&&e.photo&&(S=e.photo);let{data:t,error:n}=await i.from(`guides`).update({...T,photo:S||null}).eq(`id`,L).select().single();if(y(),n||!t){alert(`Erreur lors de la mise à jour. Réessayez.`);return}let r=c(t),a=f.findIndex(e=>e.id===L);a!==-1&&(f[a]=r);let o=document.querySelector(`[data-guide-id="${L}"]`);if(o){let e=I(r);o.replaceWith(e),e.scrollIntoView({behavior:`smooth`,block:`center`})}}else{if(!g||!/^[a-z0-9._-]+$/.test(g)){y(),alert(`Identifiant invalide : lettres minuscules, chiffres, points, tirets uniquement (sans espaces ni accents).`);return}if(!_||_.length<6){y(),alert(`Le mot de passe doit contenir au moins 6 caractères.`);return}let{data:e,error:t}=await l.auth.signUp({email:g+a,password:_});if(t||!e.user){y(),alert(`Impossible de créer le compte du guide : ${t?.message||`erreur inconnue`}.\n(Identifiant déjà utilisé ?)`);return}let{data:n,error:r}=await i.from(`guides`).insert({...T,photo:S||null,login:g,auth_user_id:e.user.id}).select().single();if(y(),r||!n){alert(`Le compte a été créé mais l'enregistrement du guide a échoué. Réessayez.`);return}let o=c(n);f.push(o);let s=document.getElementById(`panel-guides`),u=I(o);s.appendChild(u),u.scrollIntoView({behavior:`smooth`,block:`center`})}oe(),x.classList.add(`is-hidden`),V()});var pe=document.querySelectorAll(`[data-nav-tab]`),me=document.getElementById(`panel-home`),he=document.getElementById(`panel-guides-header`),ge=document.getElementById(`panel-guides`),_e=document.getElementById(`panel-reviews`);pe.forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.navTab;pe.forEach(e=>e.classList.remove(`adm-side__link--active`)),e.classList.add(`adm-side__link--active`),me.classList.add(`is-hidden`),he.classList.add(`is-hidden`),ge.classList.add(`is-hidden`),_e.classList.add(`is-hidden`),n===`home`?(me.classList.remove(`is-hidden`),ke(),Ae()):n===`reviews`?(_e.classList.remove(`is-hidden`),$()):(he.classList.remove(`is-hidden`),ge.classList.remove(`is-hidden`))})});var H=[`Janvier`,`Février`,`Mars`,`Avril`,`Mai`,`Juin`,`Juillet`,`Août`,`Septembre`,`Octobre`,`Novembre`,`Décembre`];function U(e){return(Math.round(e)/100).toLocaleString(`fr-FR`,{minimumFractionDigits:0,maximumFractionDigits:2})+` €`}function W(e){return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`}function G(e){let[t,n,r]=e.split(`-`).map(Number);return`${r} ${H[n-1].toLowerCase()} ${t}`}function ve(){let e=[];return v.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),_.forEach(t=>{t.status===`paid`&&t.amount_paid&&e.push({date:(t.created_at||``).slice(0,10),amount:t.amount_paid})}),e}function ye(){let e=ve(),t=new Date,n=W(t),r=new Date(t),i=(t.getDay()+6)%7;r.setDate(t.getDate()-i);let a=W(r),o=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-01`,s=0,c=0,l=0,u=0;e.forEach(e=>{u+=e.amount,e.date===n&&(s+=e.amount),e.date>=a&&(c+=e.amount),e.date>=o&&(l+=e.amount)});let d=(e,t)=>{let n=document.getElementById(e);n&&(n.textContent=U(t))};d(`stat-today`,s),d(`stat-week`,c),d(`stat-month`,l),d(`stat-total`,u),be()}var K=`day`;function be(){let e=document.getElementById(`stat-breakdown`);if(!e)return;let t=ve(),n={};t.forEach(e=>{if(!e.date)return;let[t,r,i]=e.date.split(`-`).map(Number),a=``,o=``,s=``;if(K===`day`)a=e.date,o=G(e.date),s=e.date;else if(K===`month`)a=`${t}-${String(r).padStart(2,`0`)}`,o=`${H[r-1]} ${t}`,s=a;else{let e=new Date(t,r-1,i),n=(e.getDay()+6)%7;e.setDate(e.getDate()-n);let c=W(e);a=c,o=`Semaine du ${G(c)}`,s=c}n[a]||(n[a]={label:o,sort:s,amount:0}),n[a].amount+=e.amount});let r=Object.values(n).sort((e,t)=>t.sort.localeCompare(e.sort));if(r.length===0){e.innerHTML=`<p class="adm-breakdown__empty">Aucun encaissement pour le moment.</p>`;return}e.innerHTML=r.map(e=>`
      <div class="adm-breakdown__row">
      ...
      </div>`).join(``)}document.querySelectorAll(`[data-stat-period]`).forEach(e=>{e.addEventListener(`click`,()=>{K=e.dataset.statPeriod,document.querySelectorAll(`[data-stat-period]`).forEach(e=>e.classList.remove(`adm-seg__btn--active`)),e.classList.add(`adm-seg__btn--active`),be()})});var xe=`Le matin`,q={year:0,month:0,editing:!1},J=!1;function Se(){let e=new Set;return g.forEach(t=>e.add(t.date)),e}function Ce(e){let t=g.find(t=>t.date===e);return t?t.id:null}function Y(){let e=document.getElementById(`call-cal-month`),t=document.getElementById(`call-cal-grid`);if(!e||!t)return;if(!q.year){let e=new Date;q.year=e.getFullYear(),q.month=e.getMonth()}e.textContent=`${H[q.month]} ${q.year}`;let n=Se(),r=W(new Date);t.innerHTML=``;let i=(new Date(q.year,q.month,1).getDay()+6)%7,a=new Date(q.year,q.month+1,0).getDate();for(let e=0;e<i;e++){let e=document.createElement(`span`);e.className=`adm-callcal__cell adm-callcal__cell--empty`,t.appendChild(e)}for(let e=1;e<=a;e++){let i=`${q.year}-${String(q.month+1).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,a=n.has(i),o=i<r,s=document.createElement(`button`);s.type=`button`,s.className=`adm-callcal__cell`,s.innerHTML=`<span>${e}</span>`,a&&s.classList.add(`adm-callcal__cell--has`),o&&s.classList.add(`adm-callcal__cell--past`),q.editing&&!o?(s.classList.add(`adm-callcal__cell--edit`),s.addEventListener(`click`,()=>we(i))):s.disabled=!0,t.appendChild(s)}}async function we(e){if(J)return;J=!0;let t=Ce(e);if(t){if(_.some(t=>t.status===`paid`&&t.slot_date===e)&&!confirm(`Ce jour a une réservation payée. Le fermer ne supprime pas la réservation. Continuer ?`)){J=!1;return}let{error:n}=await i.from(`call_slots`).delete().eq(`id`,t);n?alert(`Erreur. Réessayez.`):g=g.filter(e=>e.id!==t)}else{let{data:t,error:n}=await i.from(`call_slots`).insert({date:e,time:xe}).select().single();!n&&t?g.push(t):alert(`Erreur. Réessayez.`)}J=!1,Y()}async function Te(){if(J)return;J=!0;let e=W(new Date),t=Se(),n=new Date(q.year,q.month+1,0).getDate(),r=[];for(let i=1;i<=n;i++){let n=`${q.year}-${String(q.month+1).padStart(2,`0`)}-${String(i).padStart(2,`0`)}`;n>=e&&!t.has(n)&&r.push({date:n,time:xe})}if(r.length){let{data:e,error:t}=await i.from(`call_slots`).insert(r).select();!t&&e?g.push(...e):alert(`Erreur. Réessayez.`)}J=!1,Y()}async function Ee(){if(J)return;let e=`${q.year}-${String(q.month+1).padStart(2,`0`)}-`,t=g.filter(t=>t.date.startsWith(e));if(t.length===0||_.some(t=>t.status===`paid`&&t.slot_date.startsWith(e))&&!confirm(`Certains jours ont des réservations payées. Les fermer ne supprime pas les réservations. Continuer ?`)||!confirm(`Fermer tous les jours de ce mois ?`))return;J=!0;let n=t.map(e=>e.id),{error:r}=await i.from(`call_slots`).delete().in(`id`,n);r?alert(`Erreur. Réessayez.`):g=g.filter(t=>!t.date.startsWith(e)),J=!1,Y()}document.getElementById(`call-cal-selectall`)?.addEventListener(`click`,Te),document.getElementById(`call-cal-clearall`)?.addEventListener(`click`,Ee),document.getElementById(`call-cal-prev`)?.addEventListener(`click`,()=>{q.month--,q.month<0&&(q.month=11,q.year--),Y()}),document.getElementById(`call-cal-next`)?.addEventListener(`click`,()=>{q.month++,q.month>11&&(q.month=0,q.year++),Y()});var De=!1;document.getElementById(`call-cal-show`)?.addEventListener(`click`,()=>{De=!De;let e=document.getElementById(`call-cal-show`),t=document.getElementById(`call-cal-edit`),n=document.getElementById(`call-cal-container`),r=document.getElementById(`call-cal-hint`),i=document.getElementById(`call-cal-monthactions`);De?(n.classList.remove(`is-hidden`),t.classList.remove(`is-hidden`),e.textContent=`Masquer`,Y()):(q.editing=!1,n.classList.add(`is-hidden`),t.classList.add(`is-hidden`),t.textContent=`Modifier`,t.classList.remove(`adm-cal-toggle--on`),r?.classList.add(`is-hidden`),i?.classList.add(`is-hidden`),e.textContent=`Voir le calendrier`)}),document.getElementById(`call-cal-edit`)?.addEventListener(`click`,()=>{q.editing=!q.editing;let e=document.getElementById(`call-cal-edit`),t=document.getElementById(`call-cal-hint`),n=document.getElementById(`call-cal-monthactions`);e.textContent=q.editing?`Terminer`:`Modifier`,e.classList.toggle(`adm-cal-toggle--on`,q.editing),t&&(t.textContent=`Cliquez sur un jour pour l'ouvrir ou le fermer à la réservation.`,t.classList.toggle(`is-hidden`,!q.editing)),n?.classList.toggle(`is-hidden`,!q.editing),Y()});function Oe(){let e=document.getElementById(`call-bookings-list`),t=document.getElementById(`call-bookings-count`);if(!e)return;let n=_.filter(e=>e.status===`paid`).sort((e,t)=>(t.slot_date+t.slot_time).localeCompare(e.slot_date+e.slot_time));if(t&&(t.textContent=n.length?String(n.length):``),n.length===0){e.innerHTML=`<p class="adm-callbookings__empty">Aucun appel réservé pour le moment.</p>`;return}e.innerHTML=n.map(e=>{let t=[[`Date de l'appel`,G(e.slot_date)],[`Heure`,e.slot_time||`—`],[`Nom & prénom`,e.name||`—`],[`Nombre de personnes`,e.group_size?String(e.group_size):`—`],[`Date d'arrivée`,e.arrival?G(e.arrival):`—`],[`Date de départ`,e.departure?G(e.departure):`—`],[`Téléphone`,e.phone||`—`],[`Email`,e.email||`—`],[`Billets d'avion`,e.has_tickets?`Oui, déjà réservés`:`Non, pas encore`],[`Montant payé`,e.amount_paid?U(e.amount_paid):`—`]].map(([e,t])=>`
        <div class="adm-cb__row"><span class="adm-cb__k">${Z(e)}</span><span class="adm-cb__v">${Z(t)}</span></div>`).join(``);return`
        <div class="adm-cb" data-cb>
          <button type="button" class="adm-cb__head" data-cb-toggle>
            <span class="adm-cb__title">${Z(e.name||`Client`)}</span>
            <span class="adm-cb__meta">${Z(G(e.slot_date))} · ${Z(e.slot_time||``)}</span>
        ...
        </div>`}).join(``)}document.getElementById(`call-bookings-list`)?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-cb-toggle]`);t&&t.parentElement?.classList.toggle(`adm-cb--open`)});function ke(){ye(),Y(),Oe()}async function Ae(){let[e,t,n]=await Promise.all([i.from(`call_slots`).select(`*`).order(`date`),i.from(`call_bookings`).select(`*`).order(`slot_date`),i.from(`bookings`).select(`*`).order(`date`)]);e.data&&(g=e.data),t.data&&(_=t.data),n.data&&(v=n.data,p=n.data.map(o)),ke()}document.getElementById(`home-refresh`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`home-refresh`);e.disabled=!0;let t=e.innerHTML;e.textContent=`Actualisation…`,await Ae(),e.disabled=!1,e.innerHTML=t});var X=`pending`;function je(){return m}async function Me(e,t){let{error:n}=await i.from(`reviews`).update({status:t}).eq(`id`,e);if(n)return alert(`Erreur, réessayez.`),!1;let r=m.find(t=>t.id===e);return r&&(r.status=t),!0}function Z(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Q(){let e=je().filter(e=>e.status===`pending`),t=document.getElementById(`pending-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function Ne(){let e=N().filter(e=>e.status===`pending-refusal`),t=document.getElementById(`refusal-count`);t&&(t.textContent=String(e.length),t.hidden=e.length===0)}function $(){let e=je(),t=e.filter(e=>e.status===`pending`),n=e.filter(e=>e.status===`approved`),r=e.filter(e=>e.status===`rejected`),a=document.getElementById(`reviews-sub`);a&&(a.textContent=`${e.length} avis au total — ${t.length} en attente`);let o=document.getElementById(`rtab-pending-count`),s=document.getElementById(`rtab-approved-count`),c=document.getElementById(`rtab-rejected-count`);o&&(o.textContent=t.length>0?`(${t.length})`:``),s&&(s.textContent=n.length>0?`(${n.length})`:``),c&&(c.textContent=r.length>0?`(${r.length})`:``);let l=X===`pending`?t:X===`approved`?n:r,u=document.getElementById(`reviews-list`);if(u){if(l.length===0){u.innerHTML=`<div class="adm-reviews__empty">Aucun avis ${X===`pending`?`en attente`:X===`approved`?`approuvé`:`refusé`}.</div>`;return}u.innerHTML=l.map(e=>{let t=`★`.repeat(e.stars||5)+`☆`.repeat(5-(e.stars||5)),n=e.date?new Date(e.date).toLocaleDateString(`fr-FR`,{day:`numeric`,month:`long`,year:`numeric`}):``,r=``;return r=e.status===`pending`?`
      ...
      `}).join(``),u.querySelectorAll(`[data-rv-approve]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvApprove;await Me(t,`approved`)&&($(),Q())})}),u.querySelectorAll(`[data-rv-reject]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvReject;await Me(t,`rejected`)&&($(),Q())})}),u.querySelectorAll(`[data-rv-delete]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.rvDelete,{error:n}=await i.from(`reviews`).delete().eq(`id`,t);if(n){alert(`Erreur, réessayez.`);return}m=m.filter(e=>e.id!==t),$(),Q()})})}}document.querySelectorAll(`[data-rtab]`).forEach(e=>{e.addEventListener(`click`,()=>{X=e.dataset.rtab,document.querySelectorAll(`[data-rtab]`).forEach(e=>e.classList.remove(`adm-reviews__tab--active`)),e.classList.add(`adm-reviews__tab--active`),$()})}),Q(),Ne();